...
class Habit(models.Model):
    ...
    habit_type = models.CharField(
        ...
        default=HabitType.useful,
        verbose_name='Тип привычки'
    )
    related_habits = models.ManyToManyField(
        'self',
        symmetrical=False,
        related_name='habits',
    ...
    def __str__(self):
        return (f'Привычка: {self.action[:20]},'
                f'принадлежит user:{self.owner.tg_username}')
    ...
    def create_reminder_task(self):
        ...
        start_time = (datetime.combine(current_date, habit_time) -
                      timedelta(minutes=5))
